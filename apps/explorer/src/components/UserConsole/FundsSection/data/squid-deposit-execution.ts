...
export interface SquidDepositWalletClient {
  ...
  sendTransaction(request: { to: Address; data: Hex; value?: bigint; gas?: bigint }): Promise<Hash>;
}

export type SquidDepositSourceClient = Pick<PublicClient, "getChainId" | "readContract" | "waitForTransactionReceipt">;
...
export async function executeSquidDeposit({
  ...
  if ((await walletClient.getChainId()) !== request.sourceChainId) {
    throw new Error("Wallet network does not match the Squid source network");
  }
  if ((await sourceClient.getChainId()) !== request.sourceChainId) {
    throw new Error("Source RPC network does not match the Squid source network");
  }
  ...
  const transactionHash = await walletClient.sendTransaction({
    ...
    ...(quote.transaction.gasLimit === undefined ? {} : { gas: quote.transaction.gasLimit }),
  });
