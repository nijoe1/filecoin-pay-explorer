...
export const FundsSection = ({ account, network }: FundsSectionProps) => {
  ...
  // Every add-funds path is rendered once by FundingHost in the console
  // layout; the request names the shown token so a plain deposit opens on it.
  const { openAddFunds } = useFundingLaunch();
  const handleOpenDeposit = useCallback(
    () => openAddFunds({ depositToken: selectedToken }),
    [openAddFunds, selectedToken],
  );

  // Snapshot the withdraw token when its dialog opens so query and selector
  // updates cannot change a part-filled form's target. Keep the snapshot after
  // close so WithdrawDialog remains mounted while tracking its receipt. The
  // next open replaces the snapshot.
